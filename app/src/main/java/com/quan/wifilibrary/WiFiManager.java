...
public class WiFiManager extends BaseWiFiManager {
     ...
     * @return 配置是否成功
     */
    public boolean connectBeforeNetwork(@NonNull String ssid) {
        // 获取networkId
        int networkId = getConfigFromConfiguredNetworkIdBySsid(ssid);
        if (-1 != networkId) {
            // 保存配置
            boolean isSave = saveConfiguration();
            // 连接网络
    ...
    /**
     * wifi广播接收者，根据广播的wifi状态，然后发送handler的msg消息
     * 在广播接收类里面不能做耗时操作
     */
...
//                            break;
    ...
    /**
     * 在NetworkBroadcastReceiver类中接收wifi的广播信息，根据wifi的不同状态，通过handler发送不同的消息（此处为CallBackHandler）。
     *
     * 然后CallBackHandler中执行相关的操作。即调用监听器函数。
     */
